...
export default class ServiceOpen extends AuthenticatedBaseCommand<typeof ServiceOpen> {
  ...
  async run() {
    ...
    if (this.flags.name) {
      ...
      ux.action.start('Finding services in PD')
      const services = await this.pd.fetch('services', { params: params })
      if (services.length === 0) {
        ux.action.stop(chalk.bold.red('no services found matching ') + chalk.bold.blue(this.flags.name))
        this.exit(0)
      }
    ...
    if (service_ids.length === 0) {
      ux.action.stop(chalk.bold.red('no services specified'))
      this.exit(0)
    }
    ...
    try {
      for (const service_id of service_ids) {
        await open(`https://${domain}.pagerduty.com/services/${service_id}`)
      }
    } catch (error) {
