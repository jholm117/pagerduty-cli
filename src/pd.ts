...
export class PD {
  ...
  private spinnerCallback = (p: PD.CallbackParams) => {
    ...
    if (p.start && !this.progressState.started) {
      ...
      ux.action.start(this.progressState.format)
    ...
    if (updateStatusText) {
      ...
      ux.action.start(this.progressState.format + ' ' + statusText)
    }
    if (p.done) {
      if (this.progressState.stopSpinnerWhenDone) ux.action.stop(chalk.bold.green('done'))
